...
export default function DesignerProfile() {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      if (user?.email) {
        console.log('[profile] Fetching real projects via getDesignerDb');
        const { getDesignerDb, getDesignerPortfolio } = await import('@/app/actions');
        ...
        // Fetch custom portfolio items
        try {
          const portfolioRes = await getDesignerPortfolio();
          if (portfolioRes.portfolioItems && portfolioRes.portfolioItems.length > 0) {
            setPortfolioItems(portfolioRes.portfolioItems);
          }
        } catch (e) {
          console.error('[profile] Error fetching portfolio items:', e);
        }
      }
    };
    loadData();
  ...
  const averageRating = (() => {
    ...
    for (const p of projects) {
      ...
      if (description.includes('[FEEDBACK]')) {
        ...
        for (const line of lines) {
          ...
          if (trimmed.startsWith('[FEEDBACK]')) {
            ...
            if (match) {
              const ratingStr = match[1].split('/')[0] || '5';
              totalRating += parseFloat(ratingStr);
              count++;
              break;
            }
          }
                ...
                <section className="px-4 pt-8 pb-6 text-center" data-purpose="user-stats">
                  <div className="flex flex-col items-center mb-4">
                    ...
                    <div className="flex items-center justify-center gap-1 mb-1">
                      <h2 className="font-bold text-2xl tracking-tight">{designer?.fullName || (designer ? (user?.user_metadata?.fullName || user?.email?.split('@')[0] || '') : 'Loading...')}</h2>
                  ...
                  <div className="grid grid-cols-3 border-y border-[#262626] py-4 mb-6">
                    ...
                    <div>
                      <div className="font-bold text-lg">{averageRating || '-'}<span className="text-sm font-normal text-gray-400 ml-0.5">/5</span></div>
              ...
              <div className="p-4 bg-[#0a0a0a]">
                ...
                <section className="grid grid-cols-3 gap-0.5 bg-[#0a0a0a]" data-purpose="portfolio-showcase">
                  {[...projects, ...portfolioItems].length > 0 ? (
                    [
                      ...projects.map((p: any) => ({ ...p, __type: 'project' })),
                      ...portfolioItems.map((p: any) => ({ ...p, __type: 'portfolio' }))
                    ].map((item: any) => {
                      let firstImg = 'https://images.unsplash.com/photo-1558655146-9f40138edfeb?w=800&auto=format&fit=crop&q=60';
                      try {
                        const imgs = typeof item.images === 'string' ? JSON.parse(item.images) : item.images;
                        if (Array.isArray(imgs) && imgs.length > 0) {
                          firstImg = imgs[0].url || imgs[0];
                        }
                      } catch (e) {
                        // ignore
                      }
                      ...
                      return (
                        ...
                        >
                          <img alt={item.title} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500" src={firstImg} />
