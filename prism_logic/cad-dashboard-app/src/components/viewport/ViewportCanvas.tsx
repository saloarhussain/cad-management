...
export default function ViewportCanvas({ fileUrl, metalType, fileName, fileData, projectId, initialAnnotations = [], isReviewMode = false, isAutoRotate = false, sidebarFooter }: ViewportCanvasProps) {
  ...
  // New states for preview features
  const [isWireframeMode, setIsWireframeMode] = React.useState(false);
  const [gemColor, setGemColor] = React.useState<string>('Diamond');
  const [isMeasurementMode, setIsMeasurementMode] = React.useState(false);
  const [measurePoints, setMeasurePoints] = React.useState<THREE.Vector3[]>([]);
  const [overrideMetalType, setOverrideMetalType] = React.useState<MetalType | null>(null);
  const [meshVolume, setMeshVolume] = React.useState<number>(0);
  const [weightAlloy, setWeightAlloy] = React.useState('14k Gold');

  const getDensity = (alloy: string) => {
    switch (alloy) {
      case '14k Gold': return 0.0135;
  ...
  return (
    <div className="flex-1 w-full h-full flex flex-col md:flex-row overflow-hidden relative group font-body bg-[#0c0f0f]">
      {/* 3D Viewport */}
      <div className={`w-full min-h-[50vh] md:min-h-0 md:flex-1 relative overflow-hidden bg-black/50 transition-all duration-300 ${isFullscreen ? 'fixed inset-0 z-50 !min-h-screen' : ''}`}>
        <div className="absolute inset-0">
          ...
          <Suspense fallback={<Loader />}>
            ...
            >
              {fileData ? (
                <JewelryModelFromData
                  ...
                  metalType={overrideMetalType || metalType}
                  onPointerDown={handleModelClick}
                  wireframe={isWireframeMode}
                  onLoadStats={setMeshVolume}
                  gemColor={gemColor}
                />
              ) : fileUrl ? (
                <JewelryModel
            ...
            {/* Measurement Line & Points */}
            {isMeasurementMode && (
              <>
                {measurePoints.map((pt, i) => (
                  <Html key={`mp-${i}`} position={pt} center>
                    <div className="w-2 h-2 bg-cyan-400 rounded-full border border-[#0c0a04] shadow-[0_0_8px_rgba(34,211,238,0.8)]" />
                  </Html>
                ))}
                ...
                {measurePoints.length === 2 && (
                  <>
                    ...
                    <Html position={measurePoints[0].clone().lerp(measurePoints[1], 0.5)} center>
                      <div className="bg-cyan-500/80 backdrop-blur-md border border-cyan-400/30 text-[#0c0a04] px-2 py-0.5 rounded flex items-center justify-center font-black text-[9px] shadow-xl">
                        {measurePoints[0].distanceTo(measurePoints[1]).toFixed(2)} mm
                      </div>
                    </Html>
                  </>
                )}
            ...
            <OrbitControls
              makeDefault
              autoRotate={isAutoRotate && !isAnnotating && !activeAnnotationId && !isMeasurementMode}
              autoRotateSpeed={0.5}
              minDistance={2}
              maxDistance={15}
              enableDamping
        ...
        <div className="absolute top-4 left-1/2 -translate-x-1/2 flex flex-col items-center gap-3 md:gap-4 z-10 max-w-[calc(100%-2rem)]">
          <div className="text-center w-full overflow-x-auto no-scrollbar">
            <span className="text-[9px] font-black text-yellow-400/50 uppercase tracking-[0.2em] mb-2 block">Environment</span>
            <div className="flex flex-nowrap justify-center gap-1.5 p-1 bg-black/60 backdrop-blur-xl rounded-xl border border-white/5 min-w-max mx-auto">
          ...
          {!isReviewMode && projectId && (
            <button
              onClick={() => { setIsAnnotating(!isAnnotating); setIsMeasurementMode(false); }}
        ...
        <div className="absolute bottom-4 right-4 md:bottom-6 md:right-6 flex flex-col md:flex-row gap-3 opacity-100 md:opacity-0 group-hover:opacity-100 transition-opacity duration-500 z-10">
          <button
            onClick={() => setIsFullscreen(!isFullscreen)}
            className="bg-black/60 backdrop-blur-xl border border-white/10 p-3 md:p-4 rounded-xl md:rounded-2xl text-white hover:bg-[#ffe30c] hover:text-black transition-all active:scale-95 flex items-center justify-center gap-2 shadow-2xl"
          ...
          <button
            ...
            className="bg-black/60 backdrop-blur-xl border border-white/10 p-3 md:p-4 rounded-xl md:rounded-2xl text-white hover:bg-[#ffe30c] hover:text-black transition-all active:scale-95 flex items-center gap-2 shadow-2xl"
          >
      ...
      {/* Unified Asset Workstation Sidebar */}
      {!isReviewMode && (
        <aside className="flex-1 min-h-0 md:flex-none w-full md:w-[380px] lg:w-[420px] xl:w-[460px] bg-[#1a1c1c] md:border-l border-white/10 overflow-y-auto custom-scrollbar z-20">
          <div className="p-6 md:p-8 space-y-8 pb-12 md:pb-8">
            {/* Technical Specs Card */}
            <section className="bg-[#282a2b] md:bg-transparent rounded-2xl p-6 md:p-0 border border-white/5 md:border-none shadow-2xl md:shadow-none">
              <div className="flex items-center gap-3 mb-6">
                <div className="w-1.5 h-6 bg-[#00f2ff] rounded-full"></div>
                <h2 className="text-xs font-black uppercase tracking-[0.2em] text-white">Technical Specs</h2>
              </div>
              
              <div className="space-y-4">
                <div className="flex justify-between items-center py-4 px-4 bg-[#121414] rounded-xl border border-white/5">
                  <span className="text-[11px] font-bold text-[#cec7ab] uppercase tracking-widest">Naked Edges</span>
                  <span className="text-sm font-black text-white">0</span>
                </div>
                
                <div className="flex justify-between items-center py-4 px-4 bg-[#121414] rounded-xl border border-white/5">
                  <span className="text-[11px] font-bold text-[#cec7ab] uppercase tracking-widest">Watertight</span>
                  <span className="text-sm font-black text-[#00f2ff]">YES</span>
                </div>
                
                <div className="bg-[#121414] p-4 rounded-xl border border-white/5 space-y-4">
                  <div className="flex justify-between items-start">
                    <div className="flex flex-col gap-2">
                      <span className="text-[11px] font-bold text-[#cec7ab] uppercase tracking-widest">Est. Weight</span>
                      <select 
                        value={weightAlloy}
                        onChange={(e) => setWeightAlloy(e.target.value)}
                        className="bg-[#333535] text-[10px] font-black text-white border-white/10 rounded-lg py-1.5 pl-3 pr-8 focus:ring-[#ffe30c] focus:border-[#ffe30c] cursor-pointer"
                      >
                    ...
                    <div className="text-right">
                      <span className="text-3xl font-black text-[#ffe30c] tracking-tighter font-headline">{(meshVolume * getDensity(weightAlloy)).toFixed(2)}G</span>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <section className="space-y-5">
              <h2 className="text-[10px] font-black uppercase tracking-[0.3em] text-white/30 px-1">Manipulation Engine</h2>
              <div className="space-y-3">
                <div 
                  onClick={() => { setIsWireframeMode(!isWireframeMode); setIsAnnotating(false); setIsMeasurementMode(false); }}
                  className="flex items-center justify-between bg-[#282a2b] p-4 rounded-2xl border border-white/5 hover:border-[#00f2ff]/30 transition-colors cursor-pointer group"
                >
                  <div className="flex items-center gap-4">
                    <div className={`p-2.5 rounded-xl border border-white/5 transition-colors ${isWireframeMode ? 'bg-[#00f2ff]/10 text-[#00f2ff]' : 'bg-[#121414] group-hover:text-[#00f2ff] text-white/60'}`}>
                      <span className="material-symbols-outlined text-[20px]">{isWireframeMode ? 'visibility_off' : 'visibility'}</span>
                    </div>
                    <span className="text-xs font-black uppercase tracking-widest text-white">X-Ray Mode</span>
                  </div>
                  <button className={`w-12 h-6 rounded-full relative transition-all border ${isWireframeMode ? 'bg-[#00f2ff]/20 border-[#00f2ff]/50' : 'bg-white/10 border-transparent'}`}>
                    <span className={`absolute top-1 w-4 h-4 rounded-full transition-all ${isWireframeMode ? 'right-1 bg-[#00f2ff] shadow-[0_0_8px_rgba(0,242,255,0.5)]' : 'left-1 bg-white/40'}`}></span>
                  </button>
                </div>
                
                <div 
                  onClick={() => { setIsMeasurementMode(!isMeasurementMode); setIsAnnotating(false); setIsWireframeMode(false); setMeasurePoints([]); }}
                  className="flex items-center justify-between bg-[#282a2b] p-4 rounded-2xl border border-white/5 hover:border-[#ffe30c]/30 transition-colors cursor-pointer group"
                >
                  <div className="flex items-center gap-4">
                    <div className={`p-2.5 rounded-xl border border-white/5 transition-colors ${isMeasurementMode ? 'bg-[#ffe30c]/10 text-[#ffe30c]' : 'bg-[#121414] group-hover:text-[#ffe30c] text-white/60'}`}>
                      <span className="material-symbols-outlined text-[20px]">straighten</span>
                    </div>
                    <span className="text-xs font-black uppercase tracking-widest text-white">Measurement Tool</span>
                  </div>
                  <button className={`w-12 h-6 rounded-full relative transition-all border ${isMeasurementMode ? 'bg-[#ffe30c]/20 border-[#ffe30c]/50' : 'bg-white/10 border-transparent'}`}>
                    <span className={`absolute top-1 w-4 h-4 rounded-full transition-all ${isMeasurementMode ? 'right-1 bg-[#ffe30c] shadow-[0_0_8px_rgba(255,227,12,0.5)]' : 'left-1 bg-white/40'}`}></span>
                  </button>
                </div>
              </div>
            </section>
            ...
            <section className="space-y-4">
              <h2 className="text-[10px] font-black uppercase tracking-[0.3em] text-white/30 px-1">Surface Textures</h2>
              <div className="flex gap-5">
                 {(['gold', 'silver', 'rose'] as const).map(m => (
                   <button 
                     key={m} 
                     onClick={() => setOverrideMetalType(m)} 
                     className={`group flex flex-col items-center gap-3 transition-opacity ${(overrideMetalType || metalType) === m ? 'opacity-100' : 'opacity-40 hover:opacity-100'}`}
                   >
                     <div className={`w-14 h-14 rounded-full border-2 p-1 bg-[#333535] transition-all ${(overrideMetalType || metalType) === m ? (m === 'gold' ? 'border-[#ffe30c] shadow-[0_0_15px_rgba(255,227,12,0.3)]' : m === 'silver' ? 'border-white/50 shadow-[0_0_15px_rgba(255,255,255,0.3)]' : 'border-[#e5b3bb] shadow-[0_0_15px_rgba(229,179,187,0.3)]') : 'border-transparent hover:border-white/20'}`}>
                       <div className={`w-full h-full rounded-full ${m === 'gold' ? 'bg-gradient-to-tr from-[#d4af37] via-[#ffe30c] to-[#d4af37]' : m === 'silver' ? 'bg-gradient-to-tr from-gray-500 via-gray-200 to-gray-500' : 'bg-gradient-to-tr from-[#b76e79] via-[#e5b3bb] to-[#b76e79]'}`}></div>
                     </div>
                      <span className={`text-[10px] font-black uppercase tracking-widest ${(overrideMetalType || metalType) === m ? (m === 'gold' ? 'text-[#ffe30c]' : 'text-white') : 'text-white/60'}`}>{m}</span>
                    </button>
                  ))}
              </div>
            </section>

            {/* Gemstone Color Selector */}
            <section className="space-y-4 pt-6 border-t border-white/5">
              <h2 className="text-[10px] font-black uppercase tracking-[0.3em] text-white/30 px-1">Gemstone Settings</h2>
              <div className="flex flex-wrap gap-2.5">
                 {(['Diamond', 'Emerald', 'Ruby', 'Sapphire'] as const).map(gem => (
                    <button 
                      key={gem} 
                      onClick={() => setGemColor(gem)} 
                      className={`group flex items-center gap-2 px-3 py-2 rounded-xl bg-[#282a2b] border transition-all ${gemColor === gem ? 'border-[#ffe30c] text-white' : 'border-white/5 text-white/60 hover:border-white/20 hover:text-white'}`}
                    >
                      <span className={`w-2 h-2 rounded-full ${gem === 'Diamond' ? 'bg-cyan-100 border border-white/25' : gem === 'Emerald' ? 'bg-emerald-500' : gem === 'Ruby' ? 'bg-red-500' : 'bg-blue-600'}`} />
                      <span className="text-[9px] font-black uppercase tracking-widest leading-none">{gem}</span>
                    </button>
                  ))}
              </div>
            </section>

            {/* Dynamic Footer Injection (e.g. Download Card, File Tabs) */}
            {sidebarFooter && (
